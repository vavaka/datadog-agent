...
func TestRegisterCheckSampler(t *testing.T) {
	...
	agg := InitAggregator(nil, "", flavor.DefaultAgentFlavor)
...
func TestAddServiceCheckDefaultValues(t *testing.T) {
	resetAggregator()
	agg := InitAggregator(nil, "resolved-hostname", flavor.DefaultAgentFlavor)
...
func TestSetHostname(t *testing.T) {
	resetAggregator()
	agg := InitAggregator(nil, "hostname", flavor.DefaultAgentFlavor)
...
func TestDefaultData(t *testing.T) {
	...
	agg := InitAggregator(s, "hostname", flavor.DefaultAgentFlavor)
	start := time.Now()
	...
	series := metrics.Series{&metrics.Serie{
		Name:           fmt.Sprintf("datadog.%s.running", flavor.DefaultAgentFlavor),
		...
		Name:           fmt.Sprintf("n_o_i_n_d_e_x.datadog.%s.payload.dropped", flavor.DefaultAgentFlavor),
...
func TestRecurentSeries(t *testing.T) {
	resetAggregator()
	s := &serializer.MockSerializer{}
	agg := NewBufferedAggregator(s, "hostname", flavor.DefaultAgentFlavor, DefaultFlushInterval)
